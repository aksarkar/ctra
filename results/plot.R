...
equal_effect <- function(result_file) {
    ...
    breaks <- unique(result$p1 / 500)
    p <- (ggplot(data=result, aes(x=p1 / 500, y=prop, color=factor(comp))) +
          labs(x=expression('True causal proportion'),
               y=expression('Estimated causal proportion'), color='Annotation') +
          ...
          scale_x_continuous(breaks=breaks, trans='log10', limits=c(.001, .5)) +
          scale_y_continuous(breaks=breaks, trans='log10', limits=c(.001, .5)) +
          theme_nature +
